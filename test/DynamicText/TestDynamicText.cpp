...
auto text = DynamicText<HTMLElementArgs>(HTMLElementArgs());

void setUp()
{
  text = DynamicText<HTMLElementArgs>(HTMLElementArgs());
}
...
void test_dispatch()
{
  ...
  args.dispatch.dispatcher = [&assertions, payload](String handlerId, size_t elementId, String value) {
    assertions++;
    TEST_ASSERT_EQUAL_STRING_MESSAGE(payload.c_str(), value.c_str(), "Dispatched value should be the same (only trimmed) as provided");
  };

  text = DynamicText<HTMLElementArgs>(args);
  text.setText(payload);
  TEST_ASSERT_EQUAL_MESSAGE(1, assertions, "Should dispatch text-change event");
}
