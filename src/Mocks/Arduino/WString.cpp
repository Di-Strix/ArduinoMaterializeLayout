...
String &String::copy(const __FlashStringHelper *pstr, unsigned int length) {
    ...
    setLen(length);
    memcpy(wbuffer(), (const char*)pstr, length + 1); // We know wbuffer() cannot ever be in PROGMEM, so memcpy safe here
    return *this;
}
...
bool String::concat(const __FlashStringHelper *str) {
    ...
    if (!reserve(newlen))
        return false;
    memcpy(wbuffer() + len(), (const char*)str, length + 1);
    setLen(newlen);
    return true;
}
...
String &String::insert(size_t position, const __FlashStringHelper *other) {
    ...
    return insert(position, p, strlen(p));
}
